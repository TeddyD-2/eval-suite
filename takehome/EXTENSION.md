...
- **Axes — what they are and why.** v0 reports five axes on Google Robot pick-coke-can — orientation (3 levels), lighting (3), background (3), distractor (2), table texture (3). The reason these five: orientation and lighting are the two conditions every published VLA paper varies, so v0 numbers stay directly comparable to the SimplerEnv tables; background and table texture cover the visual-distribution shift that's the dominant failure mode for image-conditioned policies; distractor probes whether the model is attending to the right object instead of grabbing the nearest red thing. v0 also adds a paraphrase axis (5 cells including two cross-embodiment boundary cells: "walk forward to the can", "stand up and grab the can") populating the language canonical dim, and Go1's 12-cell joystick grid.
  ...
  Schema 0.3.0 (shipped alongside the v1 splat path) adds `Manifest.success_criterion` so a Task can declare its goal as a registry-keyed dict (`{"kind": "robot_reached_region", "params": {"region_name": "behind_truck", "tolerance": 0.5}}`) that's hashed into the run_id. The factory-engineer wedge follows: two sweeps of the same scene with different goals produce distinct run_ids, no new Task subclass required. The same `_LEGACY_SCHEMAS` dispatch pattern that protected the 0.1→0.2 transition protects 0.2→0.3 — every existing 0.2.0 manifest in the wild continues to verify byte-identically under its original hashing rules. The byte-identical regression is pinned as a fixture test (`tests/test_manifest_schema_evolution.py::test_schema_0_2_0_fixture_still_verifies`) so the substrate contract is enforced on every push, not just claimed.
...
- **Reproducibility.** Every run emits a `manifest.json` content-addressed by SHA256 of the canonical-JSON payload minus `run_id`; `Manifest.verify()` recomputes and asserts byte-identity. Pinned inputs: code SHA, container digest, model checkpoint hash, SimplerEnv commit (06accac), ManiSkill2_real2sim commit (ef7a4d4), Octo commit (653c54a), seed list, GPU model + CUDA + driver. Non-determinism is handled by hashing policy outputs and success outcomes, not rendered frames — MuJoCo's EGL renderer isn't bit-stable across drivers, and CUDA convolution kernels admit small numerical variance across GPU models. So the determinism claim is precise: "same inputs → same trajectory of action samples and successes, byte-identical," not "same pixels." Schema versioning: `_hashable_payload` dispatches per-feature on `schema_version` (one `frozenset` per legacy-feature class) so existing manifests continue to verify byte-identically under their original rules even after the schema bumps: 0.1.0 excludes `canonical_axis_map` from the hash, 0.2.0 includes it but excludes `success_criterion`, 0.3.0+ includes both with `success_criterion=None` omitted from the canonical JSON. CI enforces the substrate on every push — ruff + mypy --strict + pytest, including end-to-end mock sweeps that assert manifest schema, hash correctness, and CSV parsability. The v0 76-test baseline has grown to 178 across v1 + v2 (substrate-determinism fixtures, splat-annotation round-trips, predicate registry, parametric-task contract, adapter-compatibility, splat-provenance signing, synthetic-mesh boolean-extraction tests under `[splat]`, plus LeRobot/OXE interop, RGB-D + Objaverse ingest dispatch, ROS 2 TopicSpec + ProfileGate, and Pearson r + MMRV + paired_trials sidecar coverage from the v2 pillars). No GPU in CI; GPU-side numbers + the splat-on-real-mesh boolean-extraction gate are verified per release by manual sweep.
...
- Initial 3D-splat trial-generation prototype (single environment, parametric over lighting + camera pose, end-to-end through the same Task Protocol). Builds on v0's USD→MJCF ingestion substrate (the Namaqualand boulder demo in §2), which already proves the contract absorbs real-world-scan assets through one new Task + one new sidecar. The v1 splat path adds a declarative scene-annotation layer (`NamedRegion`, `SpawnPoint`, `ExtractedBody`, `scene_transform` in `scene_metadata.json` + `scene_extractions.json`) and a declarative success-predicate library (`RobotReachedRegion`, `MaintainedClearance`, `Survived` registered by KIND) so a factory engineer captures their warehouse, picks a predicate from the library, and runs a sweep — no new Task subclass. The substrate change is one `SCHEMA_VERSION` bump (0.2.0 → 0.3.0) adding `Manifest.success_criterion` with the existing `_LEGACY_SCHEMAS` dispatch pattern, keeping every existing 0.1.0/0.2.0 manifest byte-identical under its original hashing rules. Demo asset: Tanks-and-Temples "Truck" splat (CC-BY 4.0, commercial-friendly), 9 cells (3 lighting × 3 camera), `RobotReachedRegion("behind_truck", 0.5)` as the success predicate. Heavy GPU tooling (SuGaR, Nerfstudio, `manifold3d`) lives behind the `[splat]` extra in `eval-suite-stdlib`; subprocess commit SHAs land in `convert_log.json` for provenance.
