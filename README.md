...
**LeRobot / OXE interop.** Two new Policy plugins ship behind optional extras: `LeRobotPolicy` (`pip install 'eval-suite-stdlib[lerobot]'`) wraps any HuggingFace LeRobot checkpoint as an eval-suite Policy — `python -m eval_suite.cli sweep --policy lerobot --policy-arg repo_id=lerobot/smolvla-base --task <whatever>` runs without any contract change. `OXEReplayPolicy` (`pip install 'eval-suite-stdlib[oxe]'`) replays a recorded OXE/RLDS or LeRobot HF dataset episode as an open-loop policy — the interop proof that the suite ingests OXE-format episodes, and the substrate for the trajectory-level MMRV calibration path. Tests gate on the extras being present; the heavy deps stay out of the default install via the entry-points lazy-load contract. See `examples/lerobot_smolvla_pickcoke.py`.

**ROS 2 deployment + admission gate.** A sibling package `packages/eval-suite-ros2/` ships a `rclpy.lifecycle.Node` that wraps any `eval_suite.Policy` for real-robot deployment, with the observation/action ↔ topic mapping declared in a YAML `TopicSpec` (signed into a sidecar). Three reference specs for WidowX, Unitree Go1, and Franka Panda. The thin-slice piece: a `ProfileGate` refuses to transition the node to ACTIVE when the attached eval-suite profile fails a deployer-set bar (`worst_dim_min_score`, `min_calibration_tier`, `policy_family_allowlist`, `required_paired_pearson_r`). See `docs/ros2_deployment.md` and `examples/gate_smolvla_deployment.py` for the demo.

**Sim-to-real calibration with Pearson r + MMRV.** `eval_suite.calibration` ships outcome-level Pearson r (per-cell sim vs real success rates) with a 95% bootstrap CI plus trajectory-level MMRV (maximum mean relative velocity error). The registry at `calibration/real_perf.json` carries per-condition numbers from the SimplerEnv paper (Table 3 entries with `cell_axes`), and `paired_trials.json` is the on-disk sidecar a partner lab produces when they pair a sim rollout with an OXE/LeRobot trajectory. The demo `examples/calibration_pickcoke_rt1.py` reports `Pearson r = +0.875, 95% CI [+0.733, +0.970]` over 16 paired cells from the v0 RT-1 sweep — tier A reportable per EXTENSION.md §4. See `docs/calibration_writeup.md` for what the number does and doesn't tell us.
